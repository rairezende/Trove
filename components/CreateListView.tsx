  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-slate-700">Create a New List</h1>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6 flex items-center justify-between bg-slate-50 p-4 rounded-md">
            <div>
              <h3 className="font-semibold text-slate-700">List Visibility</h3>
              <p className="text-sm text-slate-500">{isPublic ? 'Anyone with the link can view.' : 'Only you can see this list.'}</p>
            </div>
            <button
              type="button"
              onClick={() => setIsPublic(!isPublic)}
              className={`${isPublic ? 'bg-indigo-600' : 'bg-slate-300'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}
            >
              <span
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 text-white font-semibold py-3 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm disabled:bg-indigo-300"
            disabled={!title.trim()}
          >
            Create List
          </button>
